...
const REGISTRY: CommandContract[] = [
  ...
  defineContract({
    schemaVersion: "0.1.0",
    id: "gef.test.timeout.tree",
    version: "1.0.0",
    executable: "node",
    args: [
      "-e",
      "const{spawn}=require('child_process');spawn(process.execPath,[\"-e\",\"setTimeout(()=>require('fs').writeFileSync('gef-timeout-grandchild.marker','x'),2500)\"],{stdio:'ignore'});setTimeout(()=>{},30000);",
    ],
    cwdMode: "PROJECT_ROOT",
    timeoutMs: 800,
    maxStdoutBytes: 4096,
