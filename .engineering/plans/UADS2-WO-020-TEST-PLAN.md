...
- executor confirms critical source identities before material edits;
- unexpected drift is reported as baseline delta;
- architecture ownership is not silently relocated.
...
- focused Vitest suites for operational events/dashboard and new truth-kernel tests;
- full repository test suite;
- corruption, stale, missing-source and reconnect negative scenarios;
- representative local benchmark with host/runtime identity disclosed;
- Windows/Linux CI evidence;
- exact-head CI, CodeQL, Dependency Review and Cross-Platform success;
- Evidence Bundle tied to implementation SHA;
